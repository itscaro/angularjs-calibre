...
export module ApiModule {
  export class Api {
    ...
    getBookById(req, res) {
      let api: Api = req.app.get("API")
      api.db.Book.findById(req.params.id, {
        include: [api.db.Author, api.db.Rating, api.db.Language, api.db.Data, api.db.Tag]
      }).then(function(book) {
        res.json(book)
    ...
    coverUrl(req, res) {
      ...
      api.db.Book.findById(req.params.id).then(function(book) {
        var cover = {
          original: api.config.calibre.path + '/' + book.path + '/cover.jpg',
        ...
        book.getData({
          ...
          data.forEach(function(_data) {
            ...
            file = {
              ...
              path: api.config.calibre.path + '/' + book.path + '/' + _data.name + '.' + _data.format.toLowerCase()
            };
